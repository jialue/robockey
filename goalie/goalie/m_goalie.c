...
PUCK FINDING
***********************************************************************************/
int find(int* adc1, int* adc2, int* count, float* location, float* goal)	// return 1 when the robot get the puck, or 0 when it is still searching for puck.
{
	if(check(PINF, 5)) return 1;		// right
	else if(check(PINF, 6)) return 2;	// mid
	else if(check(PINF, 7)) return 3;	// left
	else
	{
		...
		else
		{
			...
			//else if(goal[0] < -100)
			//{
				//if(location[2] < PI/2 && location[2] > 0)
				//{
					//ocr1b = 0*ocr1a;
					//ocr1c = 1*ocr1a;
					//OCR1B = ocr1b;
					//OCR1C = ocr1c;
				//}
				//else if(location[2] > -PI/2 && location[2] < 0)
				//{
					//ocr1b = 1*ocr1a;
					//ocr1c = 0*ocr1a;
					//OCR1B = ocr1b;
			...
			//if(goal[0] < 0)
			//{
				//if(location[2] > 0 && location[2] < PI/2)
				//{
					//OCR1B = 0;
					//OCR1C = OCR1A;
				//}
				//else if(location[2] < 0 && location[2] > -PI/2)
				//{
					//OCR1B = OCR1A;
					//OCR1C = 0;
				//}
...
int move(float* robot, float* goal)	// move to a certain place from the current location
{
	...
	if(fabs(error) > DEADBAND)
	{
		if(error < 0)	// turn left
		{
			set(PORTB, 4);
			set(PORTB, 5);
			ocr1b = 0.75*ocr1a;	// 0.5 for RUNNING BACK, 
			ocr1c = 1*ocr1a;	// 0.65 for RUNNING BACK,
			OCR1B = ocr1b;
			OCR1C = ocr1c;
		}
		else	// turn right
		{
			set(PORTB, 4);
			set(PORTB, 5);
			ocr1b = 1*ocr1a;	// 0.65 for RUNNING BACK,
			ocr1c = 0.75*ocr1a;	// 0.5 for RUNNING BACK,
			OCR1B = ocr1b;
			OCR1C = ocr1c;
		}
	}
	...
	else
	{
		set(PORTB, 4);
		set(PORTB, 5);
		OCR1B = 1*OCR1A;	// 0.75 for RUNNING BACK
		OCR1C = 1*OCR1A;	// 0.75 for RUNNING BACK
	}
	return 1;
}
...
DEFEND
***********************************************************************************/
int defend(float* location, float* goal, int found_flag)
{
	if(goal[0] < 0)
	{
		if(location[2] > 0 && location[2] < PI - 0.01)
		{
			clear(PORTB, 4);
			set(PORTB, 5);
			OCR1B = OCR1A;
		...
		else if(location[2] < 0 && location[2] > -PI + 0.01)
		{
			set(PORTB, 4);
			clear(PORTB, 5);
			OCR1B = OCR1A;
