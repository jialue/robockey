...
PUCK FINDING
***********************************************************************************/
int find(int* adc1, int* adc2, int* count)	// return 1 when the robot get the puck, or 0 when it is still searching for puck.
{
	if(!check(PIND, 7)) return 1;
	else
	{
		int ocr1a, ocr1b, ocr1c, diff, d1, d2, d_gain;
		float speed_gain, balance_gain;
		ocr1a = OCR1A;
		d_gain = 8; // 8
		speed_gain = 1; // 0.75
		//if(*count)
...
int move(float* robot, float* goal)	// move to a certain place from the current location
{
	...
	{
		OCR1B = OCR1A;	// 0.75 for RUNNING BACK
		OCR1C = 0.98*OCR1A;	// 0.75 for RUNNING BACK
	}
	return 1;
}
